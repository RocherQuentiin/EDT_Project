...
public class Cours {
    ...
    public static boolean modifierCours(Cours cours) {
        String query = "UPDATE Cours SET nom = ?, enseignant_id = ?, salle_id = ?, date = ?, heureDebut = ?, heureFin = ? WHERE id = ?";
        try (Connection connection = DataBaseConnection.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(query)) {
            ...
            preparedStatement.setInt(2, cours.getEnseignant().getId());
            preparedStatement.setInt(3, cours.getSalle().getId());
            preparedStatement.setDate(4, Date.valueOf(cours.getHoraire().getDate()));
            preparedStatement.setTime(5, Time.valueOf(cours.getHoraire().getHeureDebut()));
            preparedStatement.setTime(6, Time.valueOf(cours.getHoraire().getHeureFin()));
